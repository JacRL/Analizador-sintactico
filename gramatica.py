...
def p_condicionF(p):
    '''CONDICIONAL : IF ID MAYORQUE NUMERO DOSPUNTOS IMPRIMIR
                   | IF ID ASIGNACION ASIGNACION NUMERO DOSPUNTOS IMPRIMIR'''
    arreglo.append("Regla -> Condicion para saber si un numero es mayor o igual igual")

def p_imprimir(p):
    '''IMPRIMIR : PRINT PARENTESIS_A COMILLADOBLE NUMERO COMILLADOBLE PARENTESIS_C CONDICIONAL
...
def p_ciclofor(p):
    ...
    arreglo.append("Regla -> Definir un ciclo tipo FOR")
